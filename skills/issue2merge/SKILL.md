...
9. Loop if needed: Copilot comments → Watch Loop (Step 12a), CI fail → full cycle (Step 2)
...
12. Act on Watch Result:
   ...
   └─ ci_failed → back to step 2 (full fix cycle)
   └─ needs_fix → enter Watch Loop (Step 12a): fix → push → Copilot → re-Watch
   └─ waiting → escalate

   ⚠️ CRITICAL: Every push + Copilot re-review MUST be followed by a new Watch agent.
   Never assume comments are done after one Watch — re-Watch until `ready_to_merge`.
   The Watch Loop (Step 12a) is how we catch Copilot's response to our fixes.
```
...
| `needs_fix` | Enter Watch Loop (Step 12a): fix → push → Copilot → re-Watch. Do NOT go through full Fix→Review→Test cycle for Copilot-only fixes. |
...
### Step 12a: Watch Loop — Copilot Comment Fixes (⛔ RE-WATCH REQUIRED)

**When Watch returns `needs_fix` (Copilot review comments), do NOT go back to Step 2.**
Instead, enter this lighter-weight loop:

```
12a.1: Fix Copilot comments (via Fix sub-agent or direct edit for trivial changes)
12a.2: Reply to EVERY Copilot thread via GraphQL addPullRequestReviewThreadReply
12a.3: Resolve EVERY thread via GraphQL resolveReviewThread
12a.4: Commit + push
12a.5: Re-notify Copilot: gh pr edit {PR_NUMBER} --add-reviewer @copilot
12a.6: ⛔ Re-Watch: Spawn a NEW Watch sub-agent to monitor CI + new comments
12a.7: ACT on new Watch result:
   └─ ready_to_merge → Step 12 (merge)
   └─ needs_fix → repeat 12a.1 (loop until clean)
   └─ ci_failed → back to Step 2 (code-level fix needed)
```
...
- Every reply MUST be via GraphQL (not REST comments API) so threads stay threaded
- Every push MUST be followed by re-notify Copilot + re-Watch
- ⛔ NEVER assume comments are done after one reply/resolve cycle. Always re-Watch.
- Max 10 Watch Loop iterations (prevents infinite loops)
- Save retrospective per iteration: `memory/{YYYY-MM-DD}-{HH-MM}-{ISSUE_NUMBER}-copilot-fix-round-{N}.md`

### Step 13: Loop Protection

- Max 5 full cycles total (Step 2→4→6→8→9→11→12)
- Watch Loop (Step 12a) max 10 iterations per push
...
| Copilot Fix | `~/.openclaw/workspace/memory/{YYYY-MM-DD}-{HH-MM}-{ISSUE_NUMBER}-copilot-fix-round-{N}.md` | `2026-06-02-15-11-881-copilot-fix-round-2.md` |
