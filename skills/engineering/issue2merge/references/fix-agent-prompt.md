...
4. After fixing, add or update unit tests for the changed code
...
5. Add or update unit tests that cover all the code changes you made
6. **Lightweight test verification:** After making changes, run tests for ONLY the affected modules/files (not the full suite). This catches obvious regressions before Review. Use:
   ```bash
   cd {REPO_PATH}
...
- **Add or update unit tests** for all code changes you make
- **⛔ Do NOT run any git commands** — no checkout, pull, add, commit, or push. The Manager handles all git operations.
- **Run lightweight tests** for affected modules only (see Instructions step 6). The Review sub-agent will run the full test suite.
